...
export const addMembersinGroupController = async (req, res) => {
  try {
    ...
    if (members.length < 1 || !chatId)
      return res
        .status(500)
        .json({ success: false, message: "No members added" });
    if (!chatId)
      return res
        .status(500)
        .json({ succes: false, message: "chat id not provided" });
...
export const removememberfromGroup = async (req, res) => {
  try {
    ...
    if (!userthatwillberemoved)
      return res
        .status(404)
        .json({ success: false, message: "User not available to be removed" });
    if (!chatfound)
      return res
        .status(404)
        .json({ success: false, message: "Chat not found" });
    if (!chatfound.groupChat)
      return res
        ...
        .json({
          success: false,
          message: "No group found, it's not a group chat",
        });
    if (chatfound.creator.toString() !== req.userId.toString())
      return res
        .status(403)
...
export const attchmentController = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: false, message: "Attachments sent successfully" });
  } catch (error) {
...
export const renameGroupController = async (req, res) => {
  try {
    ...
    if (chat.creator.toString() !== req.userId.toString())
      return res
        .status(404)
        .json({ success: false, message: "Only admin allowedto rename group" });
...
export const deleteChatController = async (req, res) => {
  try {
    ...
    if (chat.groupChat && chat.creator.toString() !== req.userId.toString())
      return res
        .status(404)
        .json({ success: false, message: "Only admin is allowed to delete" });
