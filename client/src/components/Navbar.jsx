...
function Navbar() {
  ...
  const handleLogout = async () => {
    try {
      const response = await axios.delete(
        `${backendServer}/api/v1/users/logout`,
  ...
  const fetchSearchusers = async () => {
    try {
      const res = await axios.get(
        `${backendServer}/api/v1/users/search?prompt=${prompt}`,
        { withCredentials: true }
      );
      if (res.data.success) {
  ...
  const handleSendrequest = async (_id) => {
    try {
      const response = await axios.put(
        `${backendServer}/api/v1/users/sendrequest`,
        { userId: _id },
        { withCredentials: true }
      );
      if (response.data.success) {
  ...
  return (
    <div className="w-full bg-[#202C33] h-[10vh] px-2 md:px-4 py-4 flex justify-between">
      <img
        onClick={() => navigate("/")}
        src="https://cdn3d.iconscout.com/3d/premium/thumb/chat-3d-icon-download-in-png-blend-fbx-gltf-file-formats--messenger-logo-box-bubble-speech-chatting-app-communication-and-information-pack-network-icons-7845465.png?f=webp"
        alt="logo"
        className="cursor-pointer w-[15vw] h-[15vw] md:w-[4vw] md:h-[4vw] rounded-full absolute left-32 md:left-44 top-3 md:top-1"
      />
      <h1
        onClick={() => navigate("/")}
        className="block cursor-pointer text-2xl mt-2 md:mt-0 md:text-3xl font-semibold text-green-400 md:font-bold"
      >
        Happy-Chat
      </h1>
        ...
        <div className="absolute popup border-2 shadow-2xl shadow-[black] border-black top-[14%] md:top-[2%] z-[999] right-0 left-0 w-[95%] py-6 md:py-2 bg-white rounded-xl duration-[1s] search md:w-[55%] mx-auto  flex flex-col px-4 overflow-y-scroll overflow-x-hidden">
          ...
          >
            <div
              ...
              className=" cursor-pointer absolute right-1 top-0 w-[17vw] md:w-[4vw] h-[18px] md:h-[15px] rounded-full bg-black"
            ></div>
            <label
